...
def build_country_alias_map(translate_dict):
    alias_map = {}
...
COUNTRY_ALIAS_MAP = build_country_alias_map(translate_dict)
...
def main(dry_run=True, max_updates=50, csv_path=None, log_path=None):
    ...
    for country in countries:
        ...
        if normalized_country not in holidayDict:
            print(f"[WARN] {country} not in holiday CSV.")
            continue

        holiday_list = holidayDict.get(normalized_country, [])
