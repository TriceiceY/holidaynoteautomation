...
class Autocalendar:
   ...
   def format_db_date_value(self, value):
       if not value:
           return ""
   ...
   def format_db_time_value(self, value):
       if not value:
           return ""
   ...
   def get_actual_dw_records(self, start_date, end_date, done_only: bool = True):
       con, cursor = self.connect_to_db()
       try:
           query = """
               SELECT
                   ...
                   dwt.fdCategory,
                   dwt.fdBatchFile,
                   dwt.fdChecklistsProc,
                   dwt.fdTemplateDateCreated,
                   dwt.fdTemplateLastCange,
                   dwt.fdFrqSun,
                   dwt.fdFrqMon,
                   dwt.fdFrqTue,
                   dwt.fdFrqWed,
               ...
               JOIN tblcalendar_dw_database dwdb
                 ON dwdb.fdDatabaseID = dwt.fdDatabaseID
               WHERE DATE(dwr.fddDate2Show) BETWEEN ? AND ?
           """
           params = [start_date.isoformat(), end_date.isoformat()]
           if done_only:
               query += " AND dwr.fdDone = 0"
           query += " ORDER BY dwr.fddDate2Show, cnt.fdCountryName, dwr.fdRecID"
   ...
   def parse_actual_records(self, sql_rows):
       entries = []
       for entry in sql_rows:
           entries.append({
               "actual_record_id": entry.fdRecID,
               "templateID": entry.fdTemplateID,
               "record_date": self.format_db_date_value(entry.fddDate),
               "target_date": self.format_db_date_value(entry.fddDate2Show),
               "time": self.format_db_time_value(entry.fdTime),
               ...
               "trigger": "" if not entry.fdUpdateTrigger else entry.fdUpdateTrigger,
               "procedures": "" if not entry.fdUpdateProc else entry.fdUpdateProc,
               "update_type": "" if not entry.fdUpdateType else entry.fdUpdateType,
               "priority": entry.fdPriority,
               "assign": "" if not entry.fdAssign else entry.fdAssign,
               "category": "" if not entry.fdCategory else entry.fdCategory,
               "auto": "" if not entry.fdBatchFile else entry.fdBatchFile,
               "checklists_proc": "" if not entry.fdChecklistsProc else entry.fdChecklistsProc,
               "country_id": entry.fdCountryID,
               "database_id": entry.fdDatabaseID,
               "done_date": self.format_db_date_value(entry.fdDoneDate),
               "notes": "" if not entry.fdNotes else entry.fdNotes,
               "initials": "" if not entry.fdInitials else entry.fdInitials,
               "record_created_at": self.format_db_date_value(entry.fdRecDateCreated),
               "record_last_change": self.format_db_date_value(entry.fdRecLastChange),
               "template_created_at": self.format_db_date_value(entry.fdTemplateDateCreated),
               "template_last_change": self.format_db_date_value(entry.fdTemplateLastCange),
               "fdDone": entry.fdDone,
